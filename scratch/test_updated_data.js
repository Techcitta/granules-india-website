...
// 1. In annual-reports: fix 2027AnnualReport-FY13-14.pdf year to 2014
...
annRep.items.forEach(item => {
  if (item.pdf && item.pdf.includes('2027AnnualReport-FY13-14.pdf')) {
    item.year = '2014';
    item.period = 'FY 2013-14';
    item.title = 'Annual Report FY 2013-14';
  }
});
...
updatedData.forEach(cat => {
  cat.subcategories.forEach(sub => {
    ...
    sub.items.forEach(item => {
      if (item.year === '2027') {
        fy27Items.push({ cat: cat.id, sub: sub.id, title: item.title, period: item.period, pdf: item.pdf });
      }
    });
  });
});

console.log(`Total items across all investor categories: ${totalItems}`);
console.log(`Total FY27 items found: ${fy27Items.length}`);
fy27Items.forEach(i => console.log(`  [${i.sub}] (${i.period}) ${i.title} -> ${i.pdf}`));
